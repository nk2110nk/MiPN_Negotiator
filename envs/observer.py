...
class AbstractBinaryObserve(AbstractObserve, ABC):
    def __init__(self, domain, n_turn=2):
        ...
        self.my_offer = [0] * len(self.mask) * int((n_turn + 1) / 2)
        self.opp_offer = [0] * len(self.mask) * int((n_turn + 1) / 2)
        if n_turn == 1:
            self.init_observation = np.array(self.opp_offer, dtype=np.int8)
        else:
            self.init_observation = np.array(self.my_offer + self.opp_offer, dtype=np.int8)
...
class OnehotObserve2nT(AbstractBinaryObserve):
    def __init__(self, domain, n_turn):
        ...
        self.init_observation = np.array(self.my_offer + self.opp_offer + [0.], dtype=np.float32)
        self.observation = np.empty_like(self.init_observation)
    ...
    def observe(self, state):
        ...
        if 'RLAgent' in state['current_proposer']:
            ...
            self.opp_offer = one_hot + self.opp_offer[:-len(self.mask)]
        self.observation = np.array(self.my_offer + self.opp_offer + [state['relative_time']], dtype=np.float32)
        return self.observation
